  ...
  >
    ...
    >
      <div
        ...
        class="bg-orange-500 hover:bg-orange-600 text-white p-3 sm:p-4 rounded-full shadow-2xl cursor-pointer transition-all duration-200 hover:scale-105 relative"
      >
        <svg
      ...
      <div
        class="absolute bottom-full left-1/2 -translate-x-1/2 translate-y-2 group-hover:translate-y-0 sm:left-auto sm:right-0 sm:translate-x-0 mb-4 bg-white rounded-xl shadow-2xl border border-gray-100 w-80 sm:w-72 md:w-80 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300"
      >
        ...
        <div class="p-3 sm:p-4 max-h-60 overflow-y-auto">
          <div
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <span class="font-semibold text-gray-900"
              >{{ t("cart.summary") }}:</span
            >
            <span class="font-bold text-orange-600 text-base sm:text-lg">
              {{ cart.getTotalPrice.toFixed(2) }}€
            </span>
          </div>
          <button
            @click="navigateTo('/cart')"
            class="w-full bg-orange-500 cursor-pointer hover:bg-orange-600 text-white py-2 px-3 sm:py-2 sm:px-4 rounded-lg font-medium transition-colors duration-200"
          >
            {{ t("cart.viewCart") }}
          </button>
        </div>
